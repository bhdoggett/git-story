...
function parseCommitBlock(
  ...
  if (!authorMatch) {
    throw new Error(
      `Missing or invalid author in commit block ${blockIndex + 1}`
    );
  }
  ...
  const messageMatch = cleanBlock.match(
    /Message:\s*(.+?)(?=\n\n|\n\s+\w+\s+\||$)/s
  );
  if (!messageMatch) {
 ...
 */
function parseFileStats(block: string): Array<{
  filename: string;
  status: string;
  additions: number;
  deletions: number;
 ...
 */
export function transformToGitHubFormat(parsedCommits: ParsedCommit[]): any[] {
