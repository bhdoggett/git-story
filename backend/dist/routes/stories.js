...
async function analyzeAndGroupCommits(commits, globalContext) {
...
async function analyzeNewCommitsAgainstLastChapter(lastChapter, newCommits, globalContext) {
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    const contextSection = globalContext
...
LAST CHAPTER:\nTitle: ${lastChapter.title || "(untitled)"}\nSummary: ${lastChapter.summary}\nCommits:\n${lastChapter.commits
        .map((c) => `- ${c.sha}: ${c.commit.message} (author: ${c.commit.author?.name || c.commit.author?.email || "unknown"}; date: ${c.commit.author?.date || "unknown"})\n  Files: ${(c.files || [])
        .map((f) => f.filename)
        .slice(0, 8)
        .join(", ")}`)
        .join("\n")}\n
...
Consider commit messages and touched files to determine continuity. If decision is "new", propose a concise human-friendly title (2-8 words) for the new chapter in proposedTitle. If decision is "append", proposedTitle should be null. Always include all new commit SHAs in newCommitShas. Only return JSON.`;
    try {
        ...
        return {
            ...
            newCommitShas: newCommits.map((c) => c.sha),
        };
    }
}
// Helper function to generate chapter summary
async function generateChapterSummary(commits, chapterTitle, globalContext) {
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    const contextSection = globalContext
...
router.post("/generate-chapters/:repoId", async (req, res) => {
    try {
        ...
        // Get the repository
        const repo = await prisma_1.default.repo.findFirst({
        ...
        const githubResponse = await fetch(`https://api.github.com/repos/${repo.name}/commits?per_page=100`, {
            headers: {
        ...
        const chapterGroups = await analyzeAndGroupCommits(commits, globalContext);
        // Create or update story
        let story = await prisma_1.default.story.findFirst({
            where: { repoId },
            include: { chapters: true },
        });
        if (!story) {
            story = await prisma_1.default.story.create({
                data: {
                    repoId,
            ...
            await prisma_1.default.chapter.deleteMany({
                where: { storyId: story.id },
            });
            await prisma_1.default.story.update({
                where: { id: story.id },
        ...
        for (const group of chapterGroups) {
            const chapterCommits = commits.filter((c) => group.commitShas.includes(c.sha));
            const summary = await generateChapterSummary(chapterCommits, group.title, globalContext);
...
router.get("/story/:repoId", async (req, res) => {
    try {
        const { repoId } = req.params;
        const userId = req.session.userId;
        if (!userId) {
        ...
        const story = await prisma_1.default.story.findFirst({
            where: {
                repoId,
                repo: { userId },
            },
...
router.get("/story/:repoId/check-updates", async (req, res) => {
    try {
        const { repoId } = req.params;
        const userId = req.session.userId;
        const accessToken = req.session?.accessToken;
        ...
        if (!story) {
            ...
            const recentResponse = await fetch(`https://api.github.com/repos/${repo.name}/commits?per_page=1`, {
                headers: {
                    Authorization: `token ${accessToken}`,
                    Accept: "application/vnd.github.v3+json",
                },
            ...
            const hasAnyCommits = recentResponse.ok
                ? (await recentResponse.json()).length > 0
                : false;
            return res.json({
        ...
        while (!stop && page <= 3) {
            ...
            const resp = await fetch(`https://api.github.com/repos/${fullRepoName}/commits?per_page=${perPage}&page=${page}`, {
                headers: {
                    Authorization: `token ${accessToken}`,
                    Accept: "application/vnd.github.v3+json",
                },
...
router.post("/story/:repoId/analyze-updates", async (req, res) => {
    try {
        const { repoId } = req.params;
        const userId = req.session.userId;
        const accessToken = req.session?.accessToken;
        ...
        const detailedNewCommits = [];
        for (const c of newCommits) {
            try {
                const detailResp = await fetch(`https://api.github.com/repos/${fullRepoName}/commits/${c.sha}`, {
                    headers: {
                        Authorization: `token ${accessToken}`,
                        Accept: "application/vnd.github.v3+json",
                    },
        ...
        const recentLastShas = lastChapterCommitShas.slice(Math.max(0, lastChapterCommitShas.length - 10));
        const detailedLastCommits = [];
        for (const sha of recentLastShas) {
            try {
                const detailResp = await fetch(`https://api.github.com/repos/${fullRepoName}/commits/${sha}`, {
                    headers: {
                        Authorization: `token ${accessToken}`,
                        Accept: "application/vnd.github.v3+json",
                    },
        ...
        }, detailedNewCommits, story.globalContext || undefined);
        const decision = analysis.decision === "append" ? "append" : "new";
        const annotation = decision === "append"
        ...
        return res.json({
            ...
            newCommitShas: analysis.newCommitShas || detailedNewCommits.map((c) => c.sha),
            annotation,
        });
    }
    catch (error) {
...
router.post("/story/:repoId/apply-updates", async (req, res) => {
    try {
        const { repoId } = req.params;
        const userId = req.session.userId;
        const accessToken = req.session?.accessToken;
        ...
        if (!lastChapter) {
            const chapterGroups = await analyzeAndGroupCommits(detailedNewCommits, story.globalContext || undefined);
            for (const group of chapterGroups) {
                const groupedCommits = detailedNewCommits.filter((c) => group.commitShas.includes(c.sha));
                const summary = await generateChapterSummary(groupedCommits, group.title, story.globalContext || undefined);
                await prisma_1.default.chapter.create({
                    data: {
                        storyId: story.id,
                        title: group.title,
                        summary,
            ...
            const recentLastShas = lastChapter.commitShas.slice(Math.max(0, lastChapter.commitShas.length - 10));
            const detailedLastCommits = [];
            for (const sha of recentLastShas) {
                try {
                    const detailResp = await fetch(`https://api.github.com/repos/${fullRepoName}/commits/${sha}`, {
            ...
            const decision = analysis.decision === "append" ? "append" : "new";
            if (decision === "append") {
                const newCommitShas = analysis.newCommitShas && Array.isArray(analysis.newCommitShas)
                    ? analysis.newCommitShas
                    : detailedNewCommits.map((c) => c.sha);
                ...
                const allUpdatedCommits = [];
                for (const sha of updatedShas) {
                    try {
                        const resp = await fetch(`https://api.github.com/repos/${fullRepoName}/commits/${sha}`, {
                            headers: {
                                Authorization: `token ${accessToken}`,
                                Accept: "application/vnd.github.v3+json",
                            },
                        ...
                        if (resp.ok)
                            allUpdatedCommits.push(await resp.json());
                    }
                    catch (e) {
                        console.error("Error fetching commit for summary regen:", sha, e);
                    }
                }
                const newSummary = await generateChapterSummary(allUpdatedCommits, lastChapter.title || "Chapter", story.globalContext || undefined);
                ...
                for (const group of chapterGroups) {
                    const groupedCommits = detailedNewCommits.filter((c) => group.commitShas.includes(c.sha));
                    const title = group.title || analysis.proposedTitle || "New Chapter";
                    const summary = await generateChapterSummary(groupedCommits, title, story.globalContext || undefined);
                    await prisma_1.default.chapter.create({
                        data: {
                            storyId: story.id,
                            title,
        ...
        const updated = await prisma_1.default.story.findFirst({
            where: { repoId },
            include: {
                chapters: { orderBy: { createdAt: "asc" } },
            },
        ...
        return res.json({
            ...
            chapters: updated.chapters?.map((chapter) => ({
                id: chapter.id,
                title: chapter.title,
                summary: chapter.summary,
                userNotes: chapter.userNotes,
...
router.put("/chapters/:chapterId/notes", async (req, res) => {
    try {
...
router.post("/chapters/:chapterId/regenerate-summary", async (req, res) => {
    try {
        const { chapterId } = req.params;
        const userId = req.session.userId;
        if (!userId) {
        ...
        for (const sha of chapter.commitShas) {
            const response = await fetch(`https://api.github.com/repos/${chapter.story.repo.name}/commits/${sha}`, {
                headers: {
                    Authorization: `token ${accessToken}`,
                    Accept: "application/vnd.github.v3+json",
                },
        ...
        const newSummary = await generateChapterSummary(commits, chapter.title || "Chapter");
        const updatedChapter = await prisma_1.default.chapter.update({
            where: { id: chapterId },
...
router.put("/chapters/:chapterId/title", async (req, res) => {
    try {
        const { chapterId } = req.params;
...
router.put("/chapters/:chapterId/summary", async (req, res) => {
    try {
        const { chapterId } = req.params;
...
router.get("/chapters/:chapterId/commits", async (req, res) => {
    try {
        const { chapterId } = req.params;
        const userId = req.session.userId;
        const accessToken = req.session.accessToken;
...
router.get("/story/:repoId/context", async (req, res) => {
    try {
        const { repoId } = req.params;
        const userId = req.session.userId;
        if (!userId) {
...
router.put("/story/:repoId/context", async (req, res) => {
    try {
        const { repoId } = req.params;
        const { globalContext } = req.body;
        const userId = req.session.userId;
